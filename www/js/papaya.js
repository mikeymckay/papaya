...
var Papaya, RecordAudio, Router, appName, clickortouch, defaultLanguages, directoryPath, router, serverPath,
...
Papaya = (function() {
  ...
  Papaya.downloadLanguageJson = function(languages, success) {
    ...
    return fileTransfer.download(url, targetFile, (function(_this) {
  ...
  Papaya.downloadLanguageSoundFiles = function() {
    ...
    onDirsCreated = _.after(dirsToCreate.length, function() {
      ...
      return _.each(filesToCopy, function(file) {
        ...
        return fileTransfer.download(source, targetFile, (function(_this) {
          return function(data) {
  ...
  Papaya.updatePhonemes = function() {
    var extraEnglishPhonemeSounds, phonemes;
    extraEnglishPhonemeSounds = false;
    phonemes = $("#availablePhonemes").val().split(/, */);
    $('#phonemeSelector').html(_.map(phonemes, function(phoneme, index) {
      var result;
      result = "<span class='phoneme-button button " + (extraEnglishPhonemeSounds ? "extra" : "") + "'>" + phoneme + "</span> ";
      if (phoneme === "z" && $("a.language.selected").text() === "English") {
        extraEnglishPhonemeSounds = true;
        result = result + "<span style='background-color:#efefeb;' class='phoneme-button '>&nbsp;</span> ";
      }
      return result;
    }).join(""));
...
$(document).on(clickortouch, ".phoneme-button", function(event) {
  ...
  switch (Backbone.history.fragment) {
    case "joinPhonemes":
      ...
      return _.delay(function() {
        return $('#createdWords').show().boxfit();
      }, 50);
    case "listenPhonemes":
      button = $(event.target);
      phoneme = button.text();
  ...
  document.addEventListener("deviceready", function() {
    ...
    return Papaya.loadConfig({
      success: function(success) {
        ...
        console.log("Loaded config version: " + Papaya.config.version);
        if (Papaya.config.context !== "Kenya" || Papaya.config.version < 1.3) {
          console.log("New version of config/sounds available, deleting assets and reloading");
